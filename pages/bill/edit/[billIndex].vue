...
#bill-editor.page-region.fit-content
  ...
  .editor
    ...
    .menus
      ...
      //- h2.mt-0.mb-0.text-align-right = {{filterMoney(sumEntries)}}
    hr
    h3.mb-8 Bill Total
    .input
...
onMounted(() => {
  if (route.name === 'billNew') {
    process.value.menuEditorSize = 'big'
    bill.value.datetime.created = dayjs().toISOString()
  } else {
    process.value.menuEditorSize = 'small'
    const storedBill = store.value.bills[Number(route.params.billIndex)]
    Object.assign(bill, storedBill)
...
function filterMoney(amount: number) {
...
const DEBUGSAVE = () => {
  ...
  store.dispatch('updateBills', [...store.value.bills, obj])
  router.push({ name: 'split' })
}

const discard = () => {
  if (confirm('Are you sure to leave this page?')) router.go(-1)
}

const saveAndNavigate = () => {
  saveBill()
  if (route.name === 'billNew') {
    const atIndex = store.value.bills.length - 1
    router.replace({ path: `/bill/view/${atIndex}` })
  } else {
    router.replace({ path: `/bill/view/${route.params.billIndex}` })
  }
}
...
const saveBill = () => {
  ...
  if (route.name !== 'billNew') {
    ...
    bills[Number(route.params.billIndex)] = bill
    store.dispatch('updateBills', bills)
  } else {
    store.dispatch('updateBills', [...store.value.bills, bill])
  }
}
